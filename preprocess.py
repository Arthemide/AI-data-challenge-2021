...
def pre_processing(df):
    ...
    dataFrame["Date de debut de la symptomatologie"] = (
        dataFrame["Date de debut de la symptomatologie"].map(dt64_to_float).to_numpy()
    )

    # Change option to int
    dataFrame["Oxygenotherapie"] = (
        dataFrame["Oxygenotherapie"]
        .map({"Air ambiant": 1, "Moderee": 2, "Assistance respiratoire": 3, np.nan: 0})
        .to_numpy()
    )
    ...
    # Change string to int
    dataFrame["Lieu de provenance du patient"] = (
        dataFrame["Lieu de provenance du patient"]
        .map({"Domicile": 0, "EHPAD": 1, "Hopital": 2, "Autre": 3, np.nan: 4})
        .to_numpy()
    )

    # Change option to int
    dataFrame["Echographiste"] = dataFrame["Echographiste"].map(
        {
            "Forme pour l'epidemie": 1,
            "Experience d'echographie": 2,
            "Expert": 3,
            np.nan: 0,
        }
    )

    dataFrame[boolean_cols] = dataFrame[boolean_cols].apply(
        lambda value: value.map(lambda x: 1 if x == "Oui" else 0)
    )

    # Change data from string "Stade X" to int X
    dataFrame[stade_cols] = dataFrame[stade_cols].apply(
        lambda value: value.map(lambda x: int(x[-1]) if type(x) is str else x)
    )
